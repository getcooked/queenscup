  ...
  if(isStaff()){
    ...
    orders.forEach(function(o){
      if(myIds.indexOf(o.id)!==-1||o.customer===currentUser.fullName){
        ...
        else if(o.status==='serving')notifs.push({id:'my_s_'+o.id,type:'completed',icon:'fa-concierge-bell',msg:'Your order <strong>#'+o.id+'</strong> is ready for pick up!',time:o.time,action:'myorders',orderId:o.id});
...
function openSizeSelector(id){
  ...
  var sizeBtns='<div style="display:flex;gap:10px"><button class="btn" style="flex:1;flex-direction:column;padding:16px;height:auto;background:rgba(14,140,74,0.08);border:1px solid var(--border)" onclick="addToCart('+id+',\'R\')"><div style="font-size:10px;color:var(--fg-muted);margin-bottom:4px">REGULAR (16oz)</div><div style="font-size:18px;font-weight:700;color:var(--accent-light)">\u20B1'+pr.R+'</div></button><button class="btn" style="flex:1;flex-direction:column;padding:16px;height:auto;background:rgba(201,168,76,0.08);border:1px solid var(--gold-dark)" onclick="addToCart('+id+',\'L\')"><div style="font-size:10px;color:var(--fg-muted);margin-bottom:4px">LARGE (22oz)</div><div style="font-size:18px;font-weight:700;color:var(--gold-light)">\u20B1'+pr.L+'</div></button></div>';
...
function renderInventory(){
  ...
  document.getElementById('inventoryTable').innerHTML=f.map(function(p){
    ...
    return '<tr><td><div class="item-line">'+productThumbHtml(p)+'<div><div style="font-weight:600">'+escapeHtml(p.name)+bs+'</div><div style="font-size:10px;color:var(--fg-muted)">'+escapeHtml(p.desc)+'</div></div></div></td><td><span class="badge '+cb+'">'+escapeHtml(p.category)+(p.subCat?' - '+escapeHtml(p.subCat):'')+'</span></td><td style="font-size:11px">\u20B1'+pr.R+' / \u20B1'+pr.L+'</td><td><span style="font-weight:700;color:'+(stk<=lt?(stk===0?'var(--danger)':'var(--warning)'):'var(--fg)')+'">'+stk+'</span></td><td>'+st+'</td><td><div style="display:flex;gap:5px"><button class="btn btn-secondary btn-sm btn-icon" onclick="editInventoryItem('+p.id+')"><i class="fas fa-pen"></i></button><button class="btn btn-secondary btn-sm btn-icon" onclick="adjustStock('+p.id+',10)"><i class="fas fa-plus"></i></button><button class="btn btn-secondary btn-sm btn-icon" onclick="deleteInventoryItem('+p.id+')" style="color:var(--danger)"><i class="fas fa-trash"></i></button></div></td></tr>';
  }).join('');
...
function processChatResponse(input){
  ...
  else if(l.indexOf('milktea')!==-1||l.indexOf('milk tea')!==-1){var mt=products.filter(function(p){return p.category==='Milktea Series';});addBotMessage('<strong>Milktea Series:</strong><br>'+mt.map(function(p){var pr=p.prices||{R:0,L:0};return p.icon+' <strong>'+p.name+'</strong> \u2014 \u20B1'+pr.R+'/\u20B1'+pr.L+(p.bestSeller?' \uD83D\uDC51':'');}).join('<br>'),['Fruit Teas','Lemonade','Order Now']);}
  else if(l.indexOf('fruit tea')!==-1){var ft=products.filter(function(p){return p.category==='Fruit Teas';});addBotMessage('<strong>Fruit Teas:</strong><br>'+ft.map(function(p){var pr=p.prices||{R:0,L:0};return p.icon+' <strong>'+p.name+'</strong> \u2014 \u20B1'+pr.R+'/\u20B1'+pr.L;}).join('<br>'),['Milktea','Milky Fruit Jams','Order Now']);}
  else if(l.indexOf('milky fruit')!==-1||l.indexOf('jam')!==-1){var mf=products.filter(function(p){return p.category==='Milky Fruit Jams';});addBotMessage('<strong>Milky Fruit Jams:</strong><br>'+mf.map(function(p){var pr=p.prices||{R:0,L:0};return p.icon+' <strong>'+p.name+'</strong> \u2014 \u20B1'+pr.R+'/\u20B1'+pr.L;}).join('<br>'),['Fruit Teas','Milktea']);}
  else if(l.indexOf('lemonade')!==-1||l.indexOf('lemon')!==-1||l.indexOf('calamansi')!==-1){var lm=products.filter(function(p){return p.category==='Lemonade';});var subs={};lm.forEach(function(p){var sc=p.subCat||'Lemonade';if(!subs[sc])subs[sc]=[];subs[sc].push(p);});var msg='<strong>Lemonade (100% Pure Lemon Extract):</strong><br>';Object.keys(subs).forEach(function(sc){msg+='<br><em>'+sc+':</em><br>'+subs[sc].map(function(p){var pr=p.prices||{R:0,L:0};return p.icon+' <strong>'+p.name+'</strong> \u2014 \u20B1'+pr.R+'/\u20B1'+pr.L;}).join('<br>');});addBotMessage(msg,['Milktea','Coffee','Order Now']);}
  else if(l.indexOf('coffee')!==-1||l.indexOf('latte')!==-1||l.indexOf('mocha')!==-1){var cf=products.filter(function(p){return p.category==='Coffee & Non-Coffee';});addBotMessage('<strong>Coffee & Non-Coffee:</strong><br>'+cf.map(function(p){var pr=p.prices||{R:0,L:0};return p.icon+' <strong>'+p.name+'</strong> \u2014 \u20B1'+pr.R+'/\u20B1'+pr.L+(p.bestSeller?' \uD83D\uDC51':'');}).join('<br>'),['Lemonade','Sticky Milk','Order Now']);}
  else if(l.indexOf('shake')!==-1||l.indexOf('milk shake')!==-1){var sh=products.filter(function(p){return p.category==='Fruit Milk Shake';});addBotMessage('<strong>Fruit Milk Shake:</strong><br>'+sh.map(function(p){var pr=p.prices||{R:0,L:0};return p.icon+' <strong>'+p.name+'</strong> \u2014 \u20B1'+pr.R+'/\u20B1'+pr.L;}).join('<br>'),['Sticky Milk','Coffee']);}
  else if(l.indexOf('sticky')!==-1){var st=products.filter(function(p){return p.category==='Sticky Milk Drinks';});addBotMessage('<strong>Sticky Milk Drinks:</strong><br>'+st.map(function(p){var pr=p.prices||{R:0,L:0};return p.icon+' <strong>'+p.name+'</strong> \u2014 \u20B1'+pr.R+'/\u20B1'+pr.L;}).join('<br>'),['Fruit Milk Shake','Coffee','Order Now']);}
